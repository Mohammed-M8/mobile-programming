...
class Task2ViewController: UIViewController {
   ...
   // @IBOutlet var companyLabel: UILabel!
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        if let ResourceTableViewCell = navigationController?.viewControllers.first as? ResourceTableViewCell {
            ResourceTableViewCell.updateTasks()
        }
    }
    
    @IBAction func modifyButtonTapped(_ sender: UIButton) {
        let storyboard = UIStoryboard(name: "ResourceCreation", bundle: nil)
        if let entryVC = storyboard.instantiateViewController(withIdentifier: "entry2") as? Entry2ViewController {
